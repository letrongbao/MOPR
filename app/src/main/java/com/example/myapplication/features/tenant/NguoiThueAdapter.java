 ...
 */
public class NguoiThueAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    // ── Link Google Form pre-filled (số phòng + tenant ID) ─────────────────
    // '999'     → sẽ được thay bằng soPhong thực tế của phòng.
    // 'ABC_XYZ' → sẽ được thay bằng ID document NguoiThue trên Firestore.
    ...
    private void rebuildDisplay() {
        ...
        Map<String, List<NguoiThue>> groups = new LinkedHashMap<>();
        Map<String, String> roomNames = new LinkedHashMap<>(); // roomId → soPhong

        for (NguoiThue n : filtered) {
        ...
        for (String roomId : sortedRoomIds) {
            ...
            for (NguoiThue n : groups.get(roomId)) {
    ...
    private boolean matchesFilter(NguoiThue n) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_HEADER) {
            View v = inf.inflate(R.layout.item_khach_thue_header, parent, false);
            return new HeaderViewHolder(v);
        } else {
            View v = inf.inflate(R.layout.item_khach_thue_card, parent, false);
    ...
    private void bindHeader(HeaderViewHolder h, String title, int position) {
        ...
        String roomId   = null;
        String tenantId = null;   // ID document NguoiThue trên Firestore
        ...
        if (position + 1 < displayList.size()) {
            ...
            if (!next.isHeader() && next.tenant != null) {
                NguoiThue firstTenant = next.tenant;
    ...
    private void bindItem(ItemViewHolder h, NguoiThue n) {
        // Thông tin cơ bản
