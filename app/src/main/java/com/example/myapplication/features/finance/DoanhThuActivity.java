...
public class DoanhThuActivity extends AppCompatActivity {
    ...
    private List<ChiPhi> lastExpenses;
    ...
    private TextView tvSoNguoiThue;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvSoNguoiThue = findViewById(R.id.tvSoNguoiThue);
        ...
        viewModelProvider.get(ChiPhiViewModel.class)
                .getDanhSach().observe(this, list -> {
                    lastExpenses = list;
                    updateReportStats(fmt,
                            tvDoanhThuThang,
                            tvSoHDChuaTT,
    ...
    private void updateReportStats(@androidx.annotation.NonNull NumberFormat fmt,
        ...
        for (HoaDon h : lastInvoices) {
            ...
            if (selectedMonth.equals(month) && (InvoiceStatus.UNPAID.equals(st) || InvoiceStatus.PARTIAL.equals(st))) {
                chuaThu++;
            }
        }
        ...
        for (Map.Entry<String, Double> entry : invoiceTotalById.entrySet()) {
            ...
            daThuTheoHoaDonThang += Math.min(paid, entry.getValue());
        }

        if (lastExpenses != null) {
            for (ChiPhi cp : lastExpenses) {
                if (cp == null)
                    continue;
        ...
        double congNoThang = Math.max(0, tongCanThuThang - daThuTheoHoaDonThang);
        double tiLeThuTien = tongCanThuThang > 0 ? (100.0 * daThuTheoHoaDonThang / tongCanThuThang) : 0;
