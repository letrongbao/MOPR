...
public class ContractActivity extends AppCompatActivity {
    ...
    private void wireUI() {
        ...
        btnSave.setOnClickListener(v -> saveContract());
    ...
    private void bindFeesToUI(House khu) {
        ...
        boolean keepParkingChecked = currentContract != null && currentContract.hasParkingService();
        boolean keepInternetChecked = currentContract != null && currentContract.hasInternetService();
        boolean keepLaundryChecked = currentContract != null && currentContract.hasLaundryService();

        boolean hasParkingFee = parkingPrice > 0 || keepParkingChecked;
        boolean hasInternetFee = internetPrice > 0 || keepInternetChecked;
        boolean hasLaundryFee = laundryPrice > 0 || keepLaundryChecked;
        ...
        } else if (keepParkingChecked) {
            cbParkingService.setChecked(true);
        }
        if (!hasInternetFee) {
            cbInternet.setChecked(false);
        } else if (keepInternetChecked) {
            cbInternet.setChecked(true);
        }
        if (!hasLaundryFee) {
            cbLaundryService.setChecked(false);
        } else if (keepLaundryChecked) {
            cbLaundryService.setChecked(true);
        }
    ...
    private void bindRoomToUI() {
        ...
        bindBillingReminderSelection("current_month");
        cbShowNote.setChecked(false);
        applyUtilityStartInputRules();
        updateFullToolbarHeader();
    }
    ...
    private void setContractFormEditable(boolean editable) {
        ...
        setEditTextEnabled(etPersonalId, editable);
        setEditTextEnabled(etRoomNumber, editable);
        setEditTextEnabled(etMemberCount, editable);
        setEditTextEnabled(etRentAmountInput, editable);
        setEditTextEnabled(etDepositAmount, editable);
        setEditTextEnabled(etContractDate, editable);
        ...
        if (cbShowDeposit != null)
            cbShowDeposit.setEnabled(editable);
        if (cbRemind != null)
            cbRemind.setEnabled(editable);
        if (cbParkingService != null)
            cbParkingService.setEnabled(editable);
        if (cbInternet != null)
            cbInternet.setEnabled(editable);
        if (cbLaundryService != null)
            cbLaundryService.setEnabled(editable);
        if (cbShowNote != null)
            cbShowNote.setEnabled(editable);

        if (rgBillingReminder != null) {
            rgBillingReminder.setEnabled(editable);
            for (int i = 0; i < rgBillingReminder.getChildCount(); i++) {
                View child = rgBillingReminder.getChildAt(i);
                if (child != null)
        ...
        if (ivPickDate != null)
            ivPickDate.setEnabled(editable);

        for (CheckBox extraFeeCheckBox : extraFeeOptionBoxes) {
            if (extraFeeCheckBox != null)
                extraFeeCheckBox.setEnabled(editable);
        }

        setUploadEnabled(editable);
    }
    ...
    private void bindContractToUI(@NonNull Tenant c) {
        ...
        bindBillingReminderSelection(resolveBillingStartPolicy(c));
        ...
        if (layoutParkingService != null && c.hasParkingService()) {
            layoutParkingService.setVisibility(View.VISIBLE);
            parkingVisible = true;
        }
        ...
        if (cbInternet != null && c.hasInternetService()) {
            cbInternet.setVisibility(View.VISIBLE);
        }
        if (cbLaundryService != null && c.hasLaundryService()) {
            cbLaundryService.setVisibility(View.VISIBLE);
        }
    ...
    private String normalizeBillingReminder(String value) {
        ...
        if ("next_month".equals(value) || "mid_month".equals(value) || "end_month".equals(value))
            return "next_month";
        if ("current_month".equals(value) || "start_month".equals(value))
            return "current_month";
        return "current_month";
    }

    private String resolveBillingStartPolicy(@NonNull Tenant contract) {
        String policy = contract.getBillingStartPolicy();
        if (policy != null && !policy.trim().isEmpty()) {
            return normalizeBillingReminder(policy);
        }
        return normalizeBillingReminder(contract.getBillingReminderAt());
    }
    ...
    private void printContract() {
        ...
        Tenant printableContract = buildContractSnapshotForPrint();
        String html = ContractHtmlBuilder.buildContractHtml(this, printableContract, currentRoom, currentHouse);
        ...
        @NonNull
        private Tenant buildContractSnapshotForPrint() {
        Tenant printable = new Tenant();

        printable.setFullName(text(etCustomerName).isEmpty() ? currentContract.getFullName() : text(etCustomerName));
        printable.setPersonalId(text(etPersonalId).isEmpty() ? currentContract.getPersonalId() : text(etPersonalId));
        printable.setPhoneNumber(text(etPhoneInput).isEmpty() ? currentContract.getPhoneNumber() : text(etPhoneInput));
        printable.setRentalStartDate(
            text(etContractDate).isEmpty() ? currentContract.getRentalStartDate() : text(etContractDate));
        printable.setContractEndDate(currentContract.getContractEndDate());
        printable.setContractDurationMonths(
            text(etContractMonths).isEmpty()
                ? currentContract.getContractDurationMonths()
                : parseIntOrDefault(text(etContractMonths), currentContract.getContractDurationMonths()));

        double rentAmount = MoneyFormatter.getValue(etRentAmountInput);
        printable.setRentAmount(rentAmount > 0 ? (long) rentAmount : currentContract.getRentAmount());
        double depositAmount = MoneyFormatter.getValue(etDepositAmount);
        printable.setDepositAmount(depositAmount > 0 ? (long) depositAmount : currentContract.getDepositAmount());

        printable.setMemberCount(text(etMemberCount).isEmpty()
            ? currentContract.getMemberCount()
            : parseIntOrDefault(text(etMemberCount), currentContract.getMemberCount()));
        printable.setElectricStartReading(text(etElectricStartReading).isEmpty()
            ? currentContract.getElectricStartReading()
            : parseIntOrDefault(text(etElectricStartReading), currentContract.getElectricStartReading()));
        printable.setWaterStartReading(text(etWaterStartReading).isEmpty()
            ? currentContract.getWaterStartReading()
            : parseIntOrDefault(text(etWaterStartReading), currentContract.getWaterStartReading()));

        printable.setHasParkingService(cbParkingService != null && cbParkingService.isChecked());
        printable.setHasInternetService(cbInternet != null && cbInternet.isChecked());
        printable.setHasLaundryService(cbLaundryService != null && cbLaundryService.isChecked());
        printable.setVehicleCount(text(etVehicleCount).isEmpty() ? 0 : parseIntOrDefault(text(etVehicleCount), 0));
        printable.setSelectedExtraFeeNames(collectSelectedExtraFeeNames());

        return printable;
        }

        private int parseIntOrDefault(@NonNull String value, int defaultValue) {
        try {
