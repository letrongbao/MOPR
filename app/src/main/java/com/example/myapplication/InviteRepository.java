...
public class InviteRepository {
    ...
    public void createTenantInvite(@NonNull String tenantId, @NonNull String email, @NonNull String roomId, @NonNull InviteCallback cb) {
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        if (user == null) {
            cb.onError(new IllegalStateException("User not logged in"));
            return;
    ...
    public void joinByInvite(@NonNull android.content.Context context, @NonNull String tenantId, @NonNull String code, @NonNull JoinCallback cb) {
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        if (user == null) {
            cb.onError(new IllegalStateException("User not logged in"));
            return;
        ...
        inviteRef.get()
                .addOnSuccessListener(snap -> {
                    ...
                    if (role == null) role = TenantRoles.STAFF;
                    ...
                    if (TenantRoles.TENANT.equals(role)) memberDoc.put("roomId", roomId);
                    ...
                    db.runBatch(batch -> {
                        batch.set(db.collection("tenants").document(tenantId).collection("members").document(user.getUid()), memberDoc, SetOptions.merge());
