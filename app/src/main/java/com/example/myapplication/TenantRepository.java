...
public class TenantRepository {
    ...
    private void createDefaultTenant(@NonNull Context context, @NonNull FirebaseUser user, @NonNull TenantReadyCallback cb) {
                    ...
                    Tasks.whenAllSuccess(tasks)
                            .addOnSuccessListener(results -> {
                                ...
                                for (int i = 0; i < results.size(); i++) {
                                    ...
                                    if (name == null || name.trim().isEmpty()) name = tenantIds.get(i);
    ...
    private void migrateLegacyData(@NonNull String uid, @NonNull String tenantId, @NonNull Runnable onDone) {
        migrateCollection(uid, tenantId, "phong_tro", () ->
                migrateCollection(uid, tenantId, "nguoi_thue", () ->
                        migrateCollection(uid, tenantId, "hoa_don", onDone)));
    }

    private void migrateCollection(@NonNull String uid, @NonNull String tenantId, @NonNull String collectionName, @NonNull Runnable next) {
        db.collection("users").document(uid).collection(collectionName)
                ...
                .addOnSuccessListener(qs -> {
                    ...
                    for (DocumentSnapshot doc : docs) {
                        ...
                        batch.set(
                                db.collection("tenants").document(tenantId).collection(collectionName).document(doc.getId()),
                                data,
                                SetOptions.merge()
                        );
                        opCount++;
                        if (opCount >= 450) break; // keep it simple (avoid batch limit)
                    }
