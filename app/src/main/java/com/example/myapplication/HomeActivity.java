...
public class HomeActivity extends AppCompatActivity {
            ...
            public void onSuccess(java.util.List<TenantRepository.TenantInfo> tenants) {
                ...
                if (items.length == 0) {
                    ...
                    b.setItems(items, (d, which) -> {
                        ...
                        repo.setActiveTenant(HomeActivity.this, selected.id, new TenantRepository.TenantReadyCallback() {
                            @Override
                            public void onReady(String tenantId) {
                                applyRoleUI(tenantId);
                                loadStats();
                                Toast.makeText(HomeActivity.this, "Đã chuyển: " + selected.name, Toast.LENGTH_SHORT).show();
                            }

                            @Override
                            public void onError(Exception e) {
                                Toast.makeText(HomeActivity.this, "Không thể chuyển tổ chức", Toast.LENGTH_SHORT).show();
                            }
                        });
                    });
                }
    ...
    private void showOwnerTools() {
        ...
        db.collection("tenants").document(tenantId)
                ...
                .addOnSuccessListener(doc -> {
                    ...
                    String[] options = {"Mời nhân viên (STAFF)", "Mời người thuê (TENANT)", "Quản trị tổ chức", "Quản lý khu trọ"};
                    new AlertDialog.Builder(this)
                            .setTitle("Công cụ tổ chức")
                            .setItems(options, (d, which) -> {
                                if (which == 0) showCreateInviteDialog(tenantId);
                                else if (which == 1) showCreateTenantInviteDialog(tenantId);
                                else if (which == 2) {
                                    Intent it = new Intent(HomeActivity.this, OrgAdminActivity.class);
                                    it.putExtra("TENANT_ID", tenantId);
                                    startActivity(it);
                                } else {
                                    startActivity(new Intent(HomeActivity.this, KhuTroActivity.class));
                                }
                            })
                            .setNegativeButton("Đóng", null)
                            .show();
    ...
    private void showCreateInviteDialog(String tenantId) {
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Tạo mã", (d, w) -> {
                    ...
                    ensureStaffQuotaThen(tenantId, () -> new InviteRepository().createStaffInvite(tenantId, email, new InviteRepository.InviteCallback() {
                        @Override
                ...
                .addOnSuccessListener(tdoc -> {
                    ...
                    db.collection("tenants").document(tenantId).collection("members")
                            ...
                            .addOnSuccessListener(qs -> {
                                ...
                                if (current >= maxStaff) {
                                    Toast.makeText(this, "Đã vượt giới hạn nhân viên (" + maxStaff + ")", Toast.LENGTH_LONG).show();
                                    return;
                                }
                                onAllowed.run();
                            })
    ...
    private void showCreateTenantInviteDialog(String tenantId) {
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Tạo mã", (d, w) -> {
                    ...
                    new InviteRepository().createTenantInvite(tenantId, email, roomId, new InviteRepository.InviteCallback() {
                        @Override
    ...
    private void applyRoleUI(String tenantId) {
        FirebaseUser user = mAuth.getCurrentUser();
        if (user == null || tenantId == null || tenantId.trim().isEmpty()) return;
        ...
        db.collection("tenants").document(tenantId)
                ...
                .addOnSuccessListener(doc -> {
                    ...
                    if (cardTickets != null) cardTickets.setVisibility(View.VISIBLE);
                });
    }
