# Kế hoạch Nâng cấp Màn hình Quản Lý Khách Thuê

Dựa vào 2 bản thiết kế bạn vừa cung cấp, hệ thống cần đáp ứng giao diện quản lý khách thuê phân nhóm trực quan theo từng phòng, đi kèm thanh công cụ tìm kiếm, xuất Excel và các huy hiệu (badge) chức năng/giấy tờ pháp lý cho từng cá nhân khách.
...
Bởi vì Firestore là NoSQL, cấu trúc linh hoạt. Tuy nhiên, căn cứ vào thiết kế UI mới, mỗi khách thuê đang xuất hiện **4 thông tin hoàn toàn mới** mà hiện tại Model `NguoiThue.java` trong mã nguồn chưa hề có:
...
**Điều cần làm:** Chúng ta cần thêm 4 biến này vào mô hình `NguoiThue.java`. Đối với các khách thuê CŨ đang có sẵn trên Firebase, khi tải xuống ứng dụng, 4 biến này sẽ nhận giá trị mặc định là `false` (nghĩa là "Chưa đăng ký tạm trú", "Chưa đủ giấy tờ"...) mà không gây lỗi ứng dụng (Crash). Ta cũng cần thiết kế lại form (Dialog) "Thêm người thuê" để cho phép nhập 4 trạng thái này khi tạo mới.
...
> [!TIP]
> Kế hoạch đề xuất sẽ **nâng cấp và tái cấu trúc** màn hình quản lý khách thuê hiện tại thay vì giữ một dạng danh sách cuộn phẳng (flat list) đơn điệu.

### 1. Model & Data Layer
#### [MODIFY] `domain/NguoiThue.java`
- Bổ sung 4 biến logic (boolean): `isNguoiLienHe`, `isDaiDienHopDong`, `isTamTru`, `isDayDuGiayTo` cùng Getters/Setters.

### 2. Giao diện (Layouts)
#### [NEW] `layout/activity_quan_ly_khach_thue.xml` (hoặc tái cấu trúc `activity_nguoi_thue.xml`)
- Thêm thanh tìm kiếm (Search bar) và Dropdown (Spinner) "Chọn phòng".
- Thêm hệ thống nút thao tác trên cùng: `Chia sẻ excel`, `Quản lý khách cũ` (Button styling theo màu xanh/đen trong hình).
- Một `RecyclerView` chứa toàn bộ danh sách phòng và khách thuê.

#### [NEW] `layout/item_khach_thue_header.xml`
- Sinh ra dòng chữ phân khu: "Phòng 2", "Phòng 3"... bao gồm cả button "(+) Thêm" và "(🔗) Tự nhập".

#### [NEW] `layout/item_khach_thue_card.xml` (hoặc tái cấu trúc `item_nguoi_thue.xml`)
...
#### [MODIFY] `layout/dialog_them_nguoi_thue.xml`
- Bổ sung 4 Checkbox/Switch để chủ trọ tích trạng thái "Đại diện hợp đồng", "Tạm trú"... khi cập nhật thông tin khách.

### 3. Logic & Xử lý (Java)
#### [MODIFY] `features/tenant/NguoiThueAdapter.java`
- Thay vì List phẳng `List<NguoiThue>`, Adapter sẽ dùng thuật toán chuyển thành `Danh sách kết hợp (Nhóm Phòng + Danh sách KH)`.
- Triển khai **Multi-ViewType** cho RecyclerView:
  - `VIEW_TYPE_HEADER`: Hiển thị tên phòng (Ví dụ: `item_khach_thue_header`).
  - `VIEW_TYPE_ITEM`: Hiển thị chi tiết từng người trong thẻ bài.
- Gắn tính năng gọi điện thực tế vào icon gọi (`Intent.ACTION_DIAL`).

#### [MODIFY] `features/tenant/NguoiThueActivity.java`
- Đóng vai trò tổng hợp dữ liệu từ Firebase, nạp danh sách, rồi xử lý logic gom nhóm (Group By) khách hàng theo thuộc tính `idPhong`.
